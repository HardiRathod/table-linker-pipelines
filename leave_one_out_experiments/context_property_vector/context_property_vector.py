...
def compute_context_similarity(
    ...
    for prop_val_dict in kg_id_context:
        property = prop_val_dict['p']
        if not use_relevant_properties or (use_relevant_properties
                                                and
                                                is_relevant_property(col, col2, property)):
            score, best_str_match = computes_similarity(prop_val_dict['v'],
                                                             col2_string_set,
                                                             prop_val_dict['t'])
            # TODO: remove the following if condition after rltk release: 2.0.0a19
            if score < string_similarity_threshold:
                score = 0
...
def return_a_number(col2_string: str) -> float:
    col2_string_stripped = col2_string.replace('"', '')
    to_match_1 = col2_string_stripped.replace(",", "")
    numeric_col2_value = None
    try:
        if " " in to_match_1:
            split_v = to_match_1.split(" ")
            for s in split_v:
                if return_a_number(s):
                    numeric_col2_value = return_a_number(s)
        else:
            numeric_col2_value = float(to_match_1)
    except ValueError:
        return np.nan
...
def compute_quantity_similarity(quantity_1: float, quantity_2: float) -> float:
    """
    Purpose: Calculates the score between two quantities by taking the absolute difference between them and
    dividing by the max of both.
    It is then subtracted from 1.
    ...
    if quantity_1 == 0.0 and quantity_2 == 0.0:
        return 1
    quantity_score = 1 - (abs(quantity_1 - quantity_2) / max(abs(quantity_1), abs(quantity_2)))
    return quantity_score if quantity_score >= quantity_similarity_threshold else 0

def computes_similarity(
                        context_values: List[str],
                        col2_string_set: Set[str],
                        context_values_type: str) -> Tuple[float, str]:
    max_sim = 0.0
    ...
    for col2_string in col2_string_set:
        for context_value in context_values:
            if col2_string == context_value:
                ...
                current_sim = 0
                if context_values_type == 'q':
                    col2_num = return_a_number(col2_string)
                    if col2_num:
                        current_sim = compute_quantity_similarity(float(col2_num), float(context_value))
                else:
                    current_sim = similarity.hybrid.symmetric_monge_elkan_similarity(preprocess(context_value),
                                                                                     preprocess(col2_string),
...
## Running on all Semtab files
'''
semtab_files_path = 'Experiments/reduced_train_data/biotab_data/'
semtab_context_path = 'datasets/biotable_dataset/dev-output/tbl-pipeline-nn/v1/dev_context/'
result_path = 'Experiments/context_vector_train_data/'
result_path = result_path + 'biotab_data/'
#os.mkdir(result_path)
import glob
import os
semtab_files = glob.glob(semtab_files_path + '/*.csv')
for file in semtab_files:
    filename = file.split('/')[-1][:-4]
    print(filename)
    context_file = semtab_context_path + filename + '_context.jl'
    context_dict = TableContextMatches.read_context_file(context_file)
    final_column = []
    ...
    if os.path.exists(result_file):
        #continue
        pass
    #tbl = TableContextMatches()
    final_column = {}
    reverse_properties =  {}
    all_kg_id = set(input_df['kg_id'].values.tolist())
    input_df['context'] = input_df['context'].fillna("")
    for row, context, col, kg_id, kg_id_label_str, kg_id_alias_str in zip(input_df['row'],
                                                                input_df['context'],
                                                                 input_df['column'],
                                                                 input_df['kg_id'],
                                                                 input_df['kg_labels'],
                                                                 input_df['kg_aliases']):
        kg_id_context = context_dict.get(kg_id, {})
        try:
            context_values = context.split("|")
        except:
            context_values = []
        ...
        for context in context_values:
            ...
            result = compute_context_similarity(
                                       kg_id_context = kg_id_context,
                                       col = 'col',
                                       col2 = 'col2',
                                       col2_string = context_val,
                                       string_separator = ",",
                                       return_zero_similarity = False)
            # print(result)
            if kg_id == 'Q978974':
                print(result)
                print(kg_id_context)
                print(context_val)
            for k in result:
                if k['property'] in properties_val:
                    # print('Here')
                    properties_val[k['property']] = max(properties_val[k['property']], k['score'])
                else:
                    properties_val[k['property']] = k['score']
                new_dict = {}
                if k['col2_item'] in final_column:
                    past_values_k = final_column[k['col2_item']]
                    new_dict['_'+k['property']] = k['score']
                    new = {**new_dict, **past_values_k}
                    final_column[k['col2_item']] = new
                    # final_column[k['col2_item']].append([k['property'], k['score']])
                else:
                    new_dict['_'+k['property']] = k['score']
                    final_column[k['col2_item']] = new_dict


        if kg_id in final_column:
            past_values = final_column[kg_id]
            new = {**properties_val, **past_values}
            final_column[kg_id] = new
        else:
            final_column[kg_id] = properties_val


    #print(final_column)    
    final_list = [final_column.get(kg_id, {}) for kg_id, row in zip(input_df['kg_id'], input_df['row'])]    
    input_df['context_property_vector'] = final_list
    input_df.to_csv(result_file, index = False)
'''     
semtab_files_path = 'Experiments/reduced_dev_data/biotab_data/'
semtab_context_path = 'datasets/biotable_dataset/dev-output/tbl-pipeline-nn/v1/dev_context/'
result_path = 'Experiments/context_vector_dev_data/'
result_path = result_path + 'biotab_data/'
#os.mkdir(result_path)
import glob
import os
